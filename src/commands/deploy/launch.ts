...
// `deploy` group, so a bare `faable deploy` runs exactly this. Every verb
// under `deploy` is a subcommand now, this one included: the hierarchy holds
// with no special case, and nothing can fall through to a deploy by accident
// (that is how `faable deploy <app_id> secrets list` used to deploy).
export const launch: CommandModule<unknown, DeployCommandArgs> = {
  command: ['launch', '$0'],
  describe: 'Deploy the current directory (the default: `faable deploy`)',
  builder: yargs =>
    yargs
      .option('app', {
        alias: 'a',
